...
import { createIftaRecord } from '../../../api/ifta';
...
const DriverIftaCreatePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createIftaRecord(submitData);
      toast.success('IFTA record created successfully');
      navigate(`/driver/${id}`);
    } catch (error) {
      console.error('Error creating IFTA record:', error);
      toast.error(error.message || 'Error creating IFTA record');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3, gap: 2 }}>
        ...
        <Typography variant="h4">
          Create IFTA Record for {driverData?.user?.first_name} {driverData?.user?.last_name}
        </Typography>
      </Box>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Box sx={{ display: 'flex', gap: 2, mt: 4 }}>
            <Button
              ...
              sx={{ minWidth: 120 }}
            >
              {loading ? <CircularProgress size={24} /> : 'Create IFTA Record'}
            </Button>
            <Button
              variant="outlined"
...
export default DriverIftaCreatePage; 