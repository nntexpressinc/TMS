...
const DispatcherPDF = ({ dispatcher, user }) => {
  const userSection = [
  ...
  const dispatcherSection = [
    { label: 'Nickname', value: dispatcher?.nickname },
    { label: 'Employee Status', value: dispatcher?.employee_status },
    { label: 'MC Number', value: dispatcher?.mc_number },
    { label: 'Position', value: dispatcher?.position },
    { label: 'Company Name', value: dispatcher?.company_name },
    { label: 'Office', value: dispatcher?.office },
  ];
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <Text style={styles.title}>Dispatcher Details</Text>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Dispatcher Information</Text>
          {dispatcherSection.map((item, idx) => (
            <View key={idx} style={styles.row}>
              <Text style={styles.label}>{item.label}:</Text>
              <Text style={styles.value}>{item.value || 'N/A'}</Text>
            </View>
